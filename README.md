...
# GTV eBay → OnBuy Sync (manual-OnBuy variant)
...
1. An employee adds a row: **SKU** (its **digits must form a real,
   ...
   are ignored, e.g. `GTV-5012345678900`; the digits become the product's
...
4. **Creating products on OnBuy**: run the "Export OnBuy Upload" workflow
   from the Actions tab and download the `gtv-onbuy-upload` artifact — a
...
`SUPABASE_FEED_BUCKET` (optional, e.g. `gtv-feeds` — create a public
