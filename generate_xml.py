...
# ================= ONBUY API PUSH (permanently OFF for GTV) =================
# The GTV store updates OnBuy manually via CSV export - by explicit policy,
...
def sku_numeric_part(sku):
    ...
    the barcode ("GTV-5012345678900") and everything validated, exported or
...
def main():
    ...
    if missing or duplicates:
        ...
        notify.send_alert_email("GTV sheet header row needs fixing", message)
        sys.exit(1)
    ...
    # "GTV-5012345678900" and "5012345678900" count as duplicates. The FIRST
    ...
    for idx, row in batch:
        ...
        # ================= CHANGE ALERTS (GTV manual-update workflow) =====
        ...
        # Change-alert columns (GTV manual-update workflow). "Applied on
        ...
        supabase_row = {
            ...
            # GTV change-alert state, mirrored so Supabase holds the same
    ...
    # ================= CHANGE-ALERT EMAIL (GTV manual-update workflow) =================
    ...
    if change_log:
        notify.send_alert_email(
            f"GTV: {len(change_log)} product change(s) need action on OnBuy",
            "These products changed at the supplier and need a manual update on OnBuy:\n\n"
            + "\n".join(f"- SKU {sku}: {note}" for sku, note in change_log)
            + "\n\nThe same rows are highlighted in the GTV sheet (amber = change pending, "
